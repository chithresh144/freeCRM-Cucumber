...
export class CompanyPage {
    ...
    async fillCompanyDetails(context?: any) {

        if (!context.companyName) {
            context.companyName = faker.company.name();
        }
        ...
        console.log("Filled the deatils in company page and created a company with name ", name);
    }
    /*
      async verification() {
  ...
  */
    async searchCompany(context?: any) {
        //await this.page.locator(".item-text").nth(3).click()
        await this.page.getByPlaceholder("Search").click()
        await this.page.getByPlaceholder("Search").fill(context.companyName);
        await this.page.getByPlaceholder("Search").press("Enter");
        await this.page.locator(".content").nth(4).click()
        //await this.page.getByRole('link', { name: context.companyName }).isVisible()
        //await this.page.getByRole('link', { name: context.companyName }).click();
        const companyResult = this.page
            .locator('.edit-in-place-holder').nth(0)
        ...
        expect(companyNameText?.trim()).toBe(context.companyName);
        console.log(context.companyName,"has been searched")
